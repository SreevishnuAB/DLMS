...
//  res.jsonp({success:true});
  ...
  var year,prog;
  year = user.substring(user.length-5);
  prog = user.substring(5);
  console.log(user);
  console.log(`${year} ${prog}`);
...
//    console.log(hash);
    var query = (designation == 'students')?
      `insert into ${designation} values('${req.body.username}','${req.body.dummy}','${hash}','${req.body.email}','${prog}','${year}')`:
      `insert into ${designation} values('${req.body.username}','${hash}','${req.body.email}','${(req.body.dummy).substring(5)}','${(req.body.dummy).substring(0,4)}')`;
